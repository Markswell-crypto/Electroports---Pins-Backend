...
class SoundDevicesResource(Resource):
    def get(self):
        try:
            ...
            return {"sound_devices": [{"id": device.id, "name": device.name,
                                       "price": device.price, "image_url": device.image_url} for device in sound_devices]}
    ...
    def post(self):
        try:
            ...
            new_sound_device = SoundDevice(name=args['name'], price=args['price'], image_url=args['image_url'])

            db.session.add(new_sound_device)
            db.session.commit()

            return {"sound_device": {"id": new_sound_device.id, "name": new_sound_device.name,
                                     "price": new_sound_device.price, "image_url": new_sound_device.image_url}}, 201
        except Exception as e:
            db.session.rollback()
    ...
    def get(self, id):
        try:
            ...
            return {"sound_device": {"id": sound_device.id, "name": sound_device.name,
                                     "price": sound_device.price, "image_url": sound_device.image_url}}
        except Exception as e:
